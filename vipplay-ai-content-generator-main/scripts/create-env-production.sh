#!/bin/bash
# Script to create .env.production files for deployment
# These files will be committed to git and copied directly to the server

set -e

echo "Creating .env.production files for production deployment..."

# Create api-service/.env.production
cat > api-service/.env.production <<'EOF'
# Ollama Configuration
OLLAMA_BASE_URL=http://localhost:11434
DEFAULT_MODEL=ollama/qwen2.5:3b
QUALITY_MODEL=ollama/qwen2.5:3b
...
# CORS Configuration
ALLOWED_ORIGINS=http://localhost:3000,http://3.212.80.63:3000
...
echo "✅ Created api-service/.env.production"

# Create root .env.production
# Try to read MONGODB_URI from existing .env.production if it exists
MONGODB_URI_VALUE="mongodb://admin:PLACEHOLDER_PASSWORD@YOUR_IP:27017/vipcontentai"
if [ -f .env.production ]; then
  echo "📄 Found existing .env.production, reading MONGODB_URI..."
  PROD_MONGODB_URI=$(grep -E "^MONGODB_URI=" .env.production | head -1 | cut -d'=' -f2- | tr -d '"' | tr -d "'")
  if [ -n "$PROD_MONGODB_URI" ]; then
    ...
    echo "✅ Using MONGODB_URI from existing .env.production: ${MONGODB_URI_VALUE%%@*}" # Mask password
  fi
fi

cat > .env.production <<EOF
# =============================================================================
# VIPContentAI - Production Environment Variables
# =============================================================================
...
# Authentication - JWT
# -----------------------------------------------------------------------------
JWT_SECRET=change-this-to-secure-random-string-for-production-min-32-chars
JWT_ALGORITHM=HS256
JWT_EXPIRES_IN=7d
BCRYPT_ROUNDS=12
...
echo ""
echo "📝 Next steps:"
echo "1. Review and update the .env.production files with your actual production values:"
echo "   - ✅ MONGODB_URI: Already set from existing file or default"
echo "   - ⚠️  JWT_SECRET: MUST update with secure random string (openssl rand -base64 32)"
echo "   - ⚠️  AWS_SECRET_ACCESS_KEY: MUST update with your actual AWS secret"
echo "   - ⚠️  FIRECRAWL_API_KEY: Update if different"
echo "   - ⚠️  Review all URLs and IPs match your production environment"
...
echo "   - Feature Flags"
echo ""
echo "3. ⚠️  SECURITY WARNING for Production:"
echo "   - For production, it's RECOMMENDED to use GitHub Secrets for sensitive values"
echo "   - Add secrets in: Settings → Secrets and variables → Actions"
echo "   - Secrets will override .env.production values automatically"
echo "   - Sensitive values: JWT_SECRET, AWS_SECRET_ACCESS_KEY, MONGODB_URI"
echo ""
echo "4. Commit the files:"
echo "   git add .env.production api-service/.env.production"
echo "   git commit -m 'Add production environment configuration with all variables'"
echo "   git push"
echo ""
echo "5. Deploy: The workflow will automatically use these files for production deployments"
echo "   - Workflow reads .env.production and can override with GitHub Secrets"
echo "   - Triggers on push to 'main' or 'production' branch"
echo ""
echo "⚠️  Security Note:"
echo "   - Production files contain sensitive data - use GitHub Secrets for better security"
echo "   - See .github/workflows/ENV_VARIABLES.md for details on using secrets"
