# VIP-10503: Test Model Connection - Test Specification

## Feature: Test Model Connection
Automated test specification for VIP-10503

### Test Scenarios

#### Scenario: Test model connection successfully
```gherkin
Given I am logged in as a superadmin user
And FastAPI service is running and connected to Ollama
And model "llama3.1:8b" is available in Ollama
When I POST /api/admin/ai/models/test with body {"model": "llama3.1:8b"}
Then the response status should be 200 OK
And the response should contain "result" object
And result.success should be true
And result should contain "responseTime" field
And result should contain "response" field with model output
And result should contain "testedAt" timestamp
```

#### Scenario: Test model connection - model not found
```gherkin
Given I am logged in as a superadmin user
And FastAPI service is running
And model "non-existent-model" does not exist in Ollama
When I POST /api/admin/ai/models/test with body {"model": "non-existent-model"}
Then the response status should be 200 OK
And result.success should be false
And result should contain "error" field with error message
And result should contain "responseTime" field
```

#### Scenario: Test model with custom prompt
```gherkin
Given I am logged in as a superadmin user
And FastAPI service is running and connected to Ollama
When I POST /api/admin/ai/models/test with body {"model": "llama3.1:8b", "prompt": "Say hello"}
Then the response status should be 200 OK
And result.success should be true
And result.response should contain the model's response to the custom prompt
```
