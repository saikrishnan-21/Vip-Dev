# VIP-10504: Create Model Groups - Test Specification

## Feature: Create Model Groups
Automated test specification for VIP-10504

### Test Scenarios

#### Scenario: Create model group with round-robin strategy
```gherkin
Given I am logged in as a superadmin user
And MongoDB is accessible
When I POST /api/admin/ai/groups with valid group data:
  | name | "Fast Generation" |
  | description | "Models for quick content" |
  | models | ["llama3.1:8b", "llama3.1:70b"] |
  | routingStrategy | "round-robin" |
  | isActive | true |
Then the response status should be 201 Created
And the response should contain "groupId"
And the response should contain "group" object with all fields
And the group should be stored in MongoDB model_groups collection
```

#### Scenario: Create model group with priority strategy
```gherkin
Given I am logged in as a superadmin user
When I POST /api/admin/ai/groups with:
  ...
  | priority | [70, 30] |
Then the response status should be 201 Created
And the priority array length should match models array length
```

#### Scenario: Create model group - duplicate name
```gherkin
Given I am logged in as a superadmin user
And a model group named "Existing Group" already exists
When I POST /api/admin/ai/groups with name "Existing Group"
Then the response status should be 409 Conflict
And the response should contain "Model group with this name already exists"
```

#### Scenario: Create model group - invalid routing strategy
```gherkin
Given I am logged in as a superadmin user
...
#### Scenario: Create model group - missing required fields
```gherkin
Given I am logged in as a superadmin user
When I POST /api/admin/ai/groups with only {"name": "Test Group"}
Then the response status should be 400 Bad Request
And the response should list missing required fields: name, description, models, routingStrategy
```

#### Scenario: List model groups
```gherkin
Given I am logged in as a superadmin user
And at least one model group exists in MongoDB
When I GET /api/admin/ai/groups
Then the response status should be 200 OK
And the response should contain "groups" array
And the response should contain "count" field
And each group should have _id, name, models, routingStrategy fields
```

#### Scenario: Get model group by ID
```gherkin
Given I am logged in as a superadmin user
And a model group with ID "507f1f77bcf86cd799439011" exists
When I GET /api/admin/ai/groups/507f1f77bcf86cd799439011
Then the response status should be 200 OK
And the response should contain "group" object with full details
```

#### Scenario: Update model group
```gherkin
Given I am logged in as a superadmin user
And a model group exists
When I PATCH /api/admin/ai/groups/{id} with {"description": "Updated description"}
Then the response status should be 200 OK
And the response should contain updated "group" object
And the group in MongoDB should have updatedAt timestamp
```
