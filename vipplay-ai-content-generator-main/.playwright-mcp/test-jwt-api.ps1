...
try {
    $verifyResponse = Invoke-RestMethod -Uri "http://localhost:3000/api/auth/verify" -Method POST -Headers @{"Authorization" = "Bearer $token"; "Content-Type" = "application/json"}
    ...
    Write-Host "[FAIL] Token verification failed: $_" -ForegroundColor Red
}
...
try {
    $logoutResponse = Invoke-RestMethod -Uri "http://localhost:3000/api/auth/logout" -Method POST -Headers @{"Authorization" = "Bearer $token"; "Content-Type" = "application/json"}
...
try {
    $invalidResponse = Invoke-RestMethod -Uri "http://localhost:3000/api/auth/verify" -Method POST -Headers @{"Authorization" = "Bearer invalid.token.here"; "Content-Type" = "application/json"}
    Write-Host "[FAIL] Should have failed with invalid token" -ForegroundColor Red
} catch {
...
try {
    $noTokenResponse = Invoke-RestMethod -Uri "http://localhost:3000/api/auth/verify" -Method POST -Headers @{"Content-Type"="application/json"}
    Write-Host "[FAIL] Should have failed without token" -ForegroundColor Red
} catch {
    if ($_.Exception.Response.StatusCode -eq 401) {
        Write-Host "[PASS] Correctly rejected request without token (401)" -ForegroundColor Green
    } else {
        Write-Host "[FAIL] Unexpected error: $_" -ForegroundColor Red
    }
}
...
try {
    $refresh2Response = Invoke-RestMethod -Uri "http://localhost:3000/api/auth/refresh" -Method POST -Headers @{"Authorization" = "Bearer $newToken"; "Content-Type" = "application/json"}
    Write-Host "[PASS] Token refresh with new token successful" -ForegroundColor Green
